...
class GetInstagramProfile:
    ...
    def download_users_posts_with_periods(self, username):
        ...
        SINCE = datetime(2023, 6, 23)
        UNTIL = datetime(2023, 8, 23)
    ...
    def get_users_followers(self, user_name):
        ...
        self.L.login(input("clicks_by_subhash "), input("Subhash@24 "))
...
def download_profile():
    ...
    cls.download_users_posts_with_periods(username)
    cls.download_hastag_posts(username)
    cls.get_users_followers(username)
    cls.get_users_followings(username)
    cls.get_post_comments(username)
    cls.get_post_info_csv(username)
    return render_template('index.html', output=f"Profile downloaded for {username}")
